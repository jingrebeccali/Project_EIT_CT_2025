...
n_el = 64                # Number of electrodes, for some reason, the number of electrodes must be a multiple of 4
...
mesh_obj = mesh.create(
    ...
    p_fix   = p_fix,         # points fixed on the contour. This is where you choose the placement of the electrodes
    fd      = circle,         
    fh      = area_uniform,
)
pts= mesh_obj.node     
...
ax1.set_title(r"JAC reconstruit $\Delta\sigma$")
ax1.axis("off")

div1 = make_axes_locatable(ax1)
cax1 = div1.append_axes("right", size="5%", pad=0.05)
cb1  = plt.colorbar(pc1, cax=cax1)
cb1.set_label("Δσ (unités arbitraires)")
plt.tight_layout()
plt.show()